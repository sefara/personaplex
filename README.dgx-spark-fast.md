...
Model weights and `HF_TOKEN` are never needed during build. The first server start downloads weights from Hugging Face into a runtime volume; accept the model license before starting. Keep secrets out of Git and image. This Dockerfile builds an ARM64 image; neither build nor static checks establish audio performance. The PyTorch cu130 dependency `nvidia-cusparselt-cu13 0.8.1` has an ARM64 wheel whose internal platform tag triggers one `pip check` warning. The build accepts only that exact warning after checking that its shared libraries are AArch64 ELF; every other dependency error remains fatal.
